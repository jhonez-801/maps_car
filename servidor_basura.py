...
def ajustar_a_carreteras(historial):
    """
    Toma una lista de coordenadas [[lat, lng], ...] y consulta la API 
    pública de OSRM Map Matching para devolver los puntos ajustados a las calles reales.
    """
    if not historial or len(historial) < 2:
        return historial

    # OSRM espera formato lon,lat separado por comas
    coords_str = ";".join([f"{lng},{lat}" for lat, lng in historial])
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "matchings" in data and len(data["matchings"]) > 0:
                geojson_coords = data["matchings"][0]["geometry"]["coordinates"]
                return [[coord[1], coord[0]] for coord in geojson_coords]
    except Exception as e:
        print(f"Error conectando con OSRM Map Matching: {e}")
...
def actualizar_gps():
    ...
    if historial:
        ...
        distancia_metros = R * 2 * math.atan2(math.sqrt(a), math.sqrt(1-a))
...
def obtener_vehiculos():
    # Creamos una copia para procesar el matching con OSRM sin alterar la base cruda
    vehiculos_procesados = {}
    for v_id, data in vehiculos_estado.items():
        v_copia = data.copy()
        if data.get('historial') and len(data['historial']) >= 2:
            # Ajusta los puntos brutos a la geometría real de las calles usando OSRM Map Matching
            v_copia['historial'] = ajustar_a_carreteras(data['historial'])
        vehiculos_procesados[v_id] = v_copia
        
    return jsonify(vehiculos_procesados)
